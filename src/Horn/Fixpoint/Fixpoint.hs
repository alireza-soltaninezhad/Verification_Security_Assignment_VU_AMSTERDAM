...
----------------------------------------------
post phi vars = (\vs -> substVars vs vars phi) <$> freshVars (length vars)
...
---------------------------------------------
pred_abs phi preds = map fst . filter snd . zip preds <$> mapM (implies phi) preds

-------------------------------------------------------------------------------------
fixpoint_step :: [Predicate] -> Solution -> WorkItem -> Horn a -> SolveM WorkSet
-------------------------------------------------------------------------------------
fixpoint_step preds sol w h = do
  concretePostcondition <- generateConcretePostCondition sol w h
  abstractPostCondition <- generateAbstractPostCondition h preds concretePostcondition
  generateNewWorkSet sol h abstractPostCondition

getHornVarsExceptHead :: Horn a -> [Var]
getHornVarsExceptHead h = Set.toList $ (get_vars_horn h) Set.\\ (Set.fromList (vars (hd h)))

generateConcretePostCondition :: Solution -> WorkItem -> Horn a -> SolveM Base
generateConcretePostCondition sol w h
  = post (And (base h : map updateQuery (bd h))) (getHornVarsExceptHead h)
  where
    updateQuery :: Query -> Base
    updateQuery q = if name q == query w
                       then state w
                       else plugin sol q

generateAbstractPostCondition :: Horn a -> [Predicate] -> Base -> SolveM Base
generateAbstractPostCondition h preds concretePostcondition = do
  let headVars = vars $ hd h
  let newPreds = map (\p -> substVars headVars (Set.toList $ get_vars p) p) preds
  And <$> pred_abs concretePostcondition newPreds

generateNewWorkSet :: Solution -> Horn a -> Base -> SolveM WorkSet
generateNewWorkSet sol h abstractPostCondition = do
  let hornHd = hd h
  let prevSol = fromJust $ Map.lookup (name hornHd) sol
  let normalizedAbstPost = substVars (fst prevSol) (vars hornHd) abstractPostCondition
  isSolution <- implies normalizedAbstPost (Or $ Set.toList $ snd prevSol)

  case isSolution of
    True -> pure Set.empty
    False -> pure $ Set.singleton (Item normalizedAbstPost (name hornHd))
