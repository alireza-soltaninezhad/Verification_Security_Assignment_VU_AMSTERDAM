...
-----------------------------------------------------------------------------------
subst e x query = Logic.Query (Logic.name query) . fmap (Logic.subst_exp e x) . Logic.vars $ query

-----------------------------------------------------------------------------------
generateStmtVC :: Nano.Stmt -> Logic.Query -> VCM Logic.Query
-----------------------------------------------------------------------------------
generateStmtVC Nano.Skip post = pure post
generateStmtVC (Nano.Assign x e) post = pure $ subst (Nano.expToBase e) (Logic.Var x) post

generateStmtVC (Nano.Seq s1 s2) post = generateStmtVC s2 post >>= generateStmtVC s1
generateStmtVC (Nano.SeqList []) post = pure post
generateStmtVC (Nano.SeqList (s:ss)) post = generateStmtVC (Nano.SeqList ss) post >>= generateStmtVC s

generateStmtVC (Nano.If b s1 s2) post = do
        q <- freshQuery (Logic.vars post)
        wpS1 <- generateStmtVC s1 post
        wpS2 <- generateStmtVC s2 post
        addVC $ Logic.Horn wpS1 [q] (Nano.bexpToBase b) []
        addVC $ Logic.Horn wpS2 [q] (Logic.Neg (Nano.bexpToBase b)) []
        pure q

generateStmtVC (Nano.While ps b s) post = do
        q <- freshQuery (Logic.vars post)
        wp <- generateStmtVC s q
        addVC $ Logic.Horn wp [q] (Nano.bexpToBase b) ps
        addVC $ Logic.Horn post [q] (Logic.Neg (Nano.bexpToBase b)) ps
        pure q

generateStmtVC (Nano.Assert phi) post = post <$ addBound (Logic.Bound [post] Logic.Tr phi)

generateStmtVC (Nano.Assume phi) post = do
        q <- freshQuery (Logic.vars post)
        addVC $ Logic.Horn post [q] phi []
        pure q
