...
-------------------------------------
normalizeVar e = (\v -> (v, Eq v e)) <$> nxVar

-----------------------------------------------
normalizeQuery :: Query -> NM (Query, [Base])
-----------------------------------------------
normalizeQuery p = first (Query (name p)) . unzip <$> mapM normalizeVar (vars p)

-----------------------------------------------
normalizeHorn :: Horn a -> NM (Horn a)
-----------------------------------------------
normalizeHorn h = do
  (headQuery, headBases) <- normalizeQuery (hd h)
  (bodyQueries, bodyBases) <- unzip <$> mapM normalizeQuery (bd h)
  pure $ Horn headQuery bodyQueries (And (headBases ++ [base h] ++ concat bodyBases)) (annot h)
...
normalizeBoundM b = do
  (newQueries, bases) <- unzip <$> mapM normalizeQuery (queries b)
  pure $ Bound newQueries (And (bbase b : concat bases)) (bound b)
